...
const MerchandisingSection = ({ formData, setFormData }) => {
  ...
  const {
    ...
    const response = await axiosInstance.get('/merchandising')
  ...
  const handleItemToggle = (itemId) => {
    setFormData((prevState) => {
      const item = merchandisingData.find((i) => i._id === itemId)
      const updatedItems = prevState.selectedMerchandising.some(
        (i) => i._id === itemId
      )
        ? prevState.selectedMerchandising.filter((i) => i._id !== itemId)
  ...
  const handleQuantityLimitExceeded = (itemId) => {
    const item = formData.selectedMerchandising.find((i) => i._id === itemId)
  ...
  const filteredItems = merchandisingData
    ? merchandisingData.filter((item) =>
    ...
    if (error)
      return (
        <p className='text-red-500 text-center py-4'>
          Error loading merchandising items. Please try again later.
        </p>
      )
          ...
          >
            ...
            <button
              ...
              className={`w-8 h-8 flex items-center justify-center ${
                formData.selectedMerchandising.some((i) => i._id === item._id)
